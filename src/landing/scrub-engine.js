...
function mountScrollWorld(container, config) {
  ...
  (config.chrome === false ? [sky, stage, copylayer, track] : [sky, scrollbar, topbar, stage, copylayer, route, hint, track]).forEach(n => container.appendChild(n));
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.tagline ? `<p class="sw-copy__tagline">${esc(s.tagline)}</p>` : '') +
  ...
  // Scroll is smoothed: `sy` chases the real scroll position in the frame loop so the
  // film, copy and pan glide instead of stepping with every wheel notch.
  let sy = window.scrollY || window.pageYOffset, dirty = true, lastFrame = 0;
  function read(y = sy) {
    const fade = CROSSFADE * vh;
    const pan = 17 - 34 * smooth((y / vh - 0.8) / 1.0);
    stage.style.transform = `translateX(${isMobile() ? 0 : pan}vw)`;
    let ci = 0;
    ...
    for (let i = 0; i < N; i++) {
      ...
      c.inert = cop <= 0.5;
      c.setAttribute('aria-hidden', String(cop <= 0.05));
    }
  ...
  function raf(now) {
    const dt = Math.min(0.05, ((now || 0) - lastFrame) / 1000) || 0.016; lastFrame = now || 0;
    // Critically damped follow: reduced motion tracks tightly, otherwise a soft glide.
    const y = window.scrollY || window.pageYOffset;
    const follow = 1 - Math.exp(-(reduce ? 20 : 10) * dt);
    sy += (y - sy) * follow;
    if (Math.abs(y - sy) < 0.3) sy = y;
    if (dirty || Math.abs(y - sy) > 0.05) { dirty = false; read(sy); }
    const eps = isMobile() ? 0.02 : 0.008;   // coarser seek step on phones = fewer decodes
    const seekFollow = 1 - Math.exp(-(reduce ? 20 : 11) * dt);
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      ...
      s.cur += (s.target - s.cur) * seekFollow;
